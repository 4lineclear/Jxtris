...
class Offsets {
     ...
     * @return An {@code int[][]}
     **/
    public int[][] get(Block type, int from, int to) {
        final int key = from * 10 + to;
        return (from - to == 2 || from - to == -2 ?
                offsets180 :        //If the difference between <from> and <to> is |2|, it must be a 180 rotation
                type == Block.I ?   //Checking if the block type is I
                        offsetsI :
                        offsets)
                .get(key);          //Returning the offsets of the correct map
    }
}
